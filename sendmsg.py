...
-R --mask-recv      mask received data from stdout
...
-Input files are assumed to be text files with line breaks of 0x0A for readability,
 these line terminators will be ignored in the sending of byte data.
-When it is desired to send a \r (0x0D) or \n (0x0A), put a literal '\r' or '\n' token(s)
...
-Use '\r' as desired for signaling end of DATA.
"""
...
def usage(complaint = None):
    usage_str = "sendmsg.py payload.txt server [port|25]\n"
    if not complaint == None:
        print(complaint)
    sys.exit(usage_str)
...
def validate_port(port):
    if not port.isdigit():
        usage(f"Error: {port} is not a number 1-65535")
    portnum = int(port)
